...
export const WalletModal: FC<WalletModalProps> = ({className = '', container = 'body'}) => {
    ...
    const handleWalletClick = useCallback(
        (event: MouseEvent, walletName: string) => {
            console.log('handleWalletClick')
            const change=async (walletName:string)=>{
                await select(walletName)
                console.log(`select successfully`)
                if (connector.deactivate){
                    const res=await connector.deactivate()
            ...
            change(walletName)
                .then(() => console.log(`${walletName} selected`))
                .catch((err) => console.log(`handleWalletClick err: ${err}`));
            handleClose(event);
        },
        [handleClose]
    );
                ...
                <div className="wallet-adapter-modal-container">
                    <div className="wallet-adapter-modal-wrapper">
                        ...
                        {installedWallets.length ? (
                            <>
                                <h1 className="wallet-adapter-modal-title">Connect a wallet on Ethereum to continue</h1>
                            ...
                            <>
                                <h1 className="wallet-adapter-modal-title">
                                    need a wallet on Ethereum to continue
                                </h1>
                                <div className="wallet-adapter-modal-middle">
                                    ...
                                    <button
                                        ...
                                        onClick={(event) => handleWalletClick(event, getStartedWallet.name)}
                                    >
                                        Get started
                                    </button>
                                </div>
